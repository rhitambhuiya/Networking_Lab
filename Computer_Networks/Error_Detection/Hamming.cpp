...
int main()
{
    ...
    string received;
    cout << endl << "Enter the data received at receiver's end(only the data bits, without parity bits): ";
    cin >> received;
    ...
    for(int i = 0; i < r + d; i++)
    {
        ...
        /*
        if data is received with parity bits stuffed, do this instead
        for(int i = 0; i < r + d; i++)
            data[i] = received[i] - '0';
        */
    }
    ...
    if(error_bit == 0)
        ...
        cout << endl << "Error is at bit position (1 based, from right): " << error_bit;
}